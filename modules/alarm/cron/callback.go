...
func Callback(event *model.Event, action *api.Action) string {
	...
	req.Param("right_value", utils.ReadableFloat(event.RightValue()))
	req.Param("step", fmt.Sprintf("%d", event.CurrentStep))
	req.Param("priority", fmt.Sprintf("P%d", event.Priority()))
	...
	req.Param("tags", tags)
	req.Param("tplname",event.TplName())
	req.Param("max", fmt.Sprintf("%d",event.MaxStep()))
